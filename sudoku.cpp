...
#define empty 0
#define N 9
bool isGridSafe (int[N][N], int row, int col, int num);
bool isEmptyLocation (int grid[N][N], int &row, int &col);

bool SolveSudoku(int grid[N][N])
{
	int row, col, num;
	if (!isEmptLocation(grid, row, col, num))
		return true;
...
bool isEmptLocation(int grid[N][N], int &row, int &col)
{
	for(row=0; row<N; row++)
		for(col=0; col<N; col++)
...
bool UsedInBox(int grid[N][N], int boxBeginRow, int BeginCol, int number)
{
	bool tf = false;
